...
def get_attrset_from_index [xml_content: string] {
    ...
    $packages | lines | each { |line|
        ...
        let full_package_name = $"($package_name)-($version)" | sanitize_name
...
# Generate nix expressions for ReaPack indexes and prefetch hashes.
# Reads newline-separated index URLs from stdin.
def main [
    --generate (-g) # Generate nix files from indexes into target directory
    --prefetch-hashes (-p) # Prefetch hashes for package sources
    --replace-hashes (-r) # Replace hashes in target directory
    --target-directory (-o): string # Set target directory for files to be generated and/or have hashes replaced. Use. "./generated" if left empty.
    --target-hash-file (-t): string # Specify a file to prefetch to and/or read hash data for replacement. Uses temp file if left empty.
    --silent (-s) # Do not create log files
] {
